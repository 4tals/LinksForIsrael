...
const getScrollableProps = (element: HTMLElement | null): ScrollProps => {
	return element? {
    isScrollableLeft: element.scrollLeft - 1 > element.clientWidth - element.scrollWidth,
...
const withScroll = (WrappedComponent: React.ComponentType<any>) => {
  const ComponentWithScroll = (props: any) => {
    ...
    useLayoutEffect(() => {
      if(!scrollableDivRef.current || !leftButtonRef.current || !rightButtonRef.current) return;
      ...
      const scrollLeft = (): void => {
        interval = setInterval(() => { scrollableDiv.scrollLeft -= 100; }, 100);
      }
      const scrollRight = (): void => {
        interval = setInterval(() => { scrollableDiv.scrollLeft += 100; }, 100);
      }
      const stopScroll = (): void => { clearInterval(interval); }
    ...
    return (
      <div className="scroll_wrapper">
            <button className={`chevron-button left ${scrollableProps.isScrollableLeft ? 'show' : 'hide'}`} ref={leftButtonRef}>
              <div className={`chevron left`} />
            </button>
            <div className="scroll_section" ref={scrollableDivRef} >
              <WrappedComponent {...props} />
            </div>
            <button className={`chevron-button right ${scrollableProps.isScrollableRight ? 'show' : 'hide'}`} ref={rightButtonRef}>
